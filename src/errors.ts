 ...
 */
export function createValidationError(message: string, error: z.ZodError, code: number = 400): Types.ValidationError {
  return { ok: false, code, status: "validation_error", message, error };
}
 ...
 */
export function isSuccess<T>(response: Types.ApiResponse<T, any>): response is Types.Success<T> & { toast: () => void; endpoint: string; method: Types.HttpMethod } {
 ...
 */
export function isError<T>(response: Types.ApiResponse<any, T>): response is Types.Errors<T> & { toast: () => void; endpoint: string; method: Types.HttpMethod } {
